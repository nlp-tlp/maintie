...
class GenerateTextSamplesCallback(Callback):  # pragma: no cover
    ...
    def on_train_batch_end(
        ...
        gen_kwargs = {
            ...
            "num_beams": pl_module.hparams.eval_beams
            if pl_module.hparams.eval_beams is not None
            else pl_module.config.num_beams,
        }
        pl_module.eval()
        ...
        if generated_tokens.shape[-1] < gen_kwargs["max_length"]:
            generated_tokens = pl_module._pad_tensors_to_max_len(
                generated_tokens, gen_kwargs["max_length"]
            )
        pl_module.train()
        decoded_preds = pl_module.tokenizer.batch_decode(
        ...
        if pl_module.hparams.ignore_pad_token_for_loss:
            ...
            labels = torch.where(
                labels != -100, labels, pl_module.tokenizer.pad_token_id
            )

        decoded_labels = pl_module.tokenizer.batch_decode(
            labels, skip_special_tokens=False
        )
        decoded_inputs = pl_module.tokenizer.batch_decode(
        ...
        for source, translation, gold_output in zip(
            decoded_inputs, decoded_preds, decoded_labels
        ):
            wandb_table.add_data(
                source.replace("<pad>", ""),
                translation.replace("<pad>", ""),
                gold_output.replace("<pad>", ""),
            )
