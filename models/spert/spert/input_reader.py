...
class BaseInputReader(ABC):
    def __init__(
        ...
        **kwargs
        ...
        # print("types", types["entities"][0].items())
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_document(self, doc, dataset) -> Document:
        ...
        document = dataset.create_document(
            doc_tokens, entities, relations, doc_encoding
        )
...
class JsonPredictionInputReader(BaseInputReader):
    def __init__(
        ...
        super().__init__(
            types_path, tokenizer, max_span_size=max_span_size, logger=logger
        )
        self._spacy_model = spacy_model

        self._nlp = (
            spacy.load(spacy_model)
            if spacy is not None and spacy_model is not None
            else None
        )
