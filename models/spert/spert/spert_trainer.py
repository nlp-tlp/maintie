...
class SpERTTrainer(BaseTrainer):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        self._tokenizer = BertTokenizer.from_pretrained(
            args.tokenizer_path, do_lower_case=args.lowercase, cache_dir=args.cache_path
        )
    ...
    def train(
        ...
        valid_path: str,
        types_path: str,
        input_reader_cls: Type[BaseInputReader],
    ):
        args = self._args
        ...
        for epoch in range(args.epochs):
            ...
            if not args.final_eval or (epoch == args.epochs - 1):
                self._eval(
                    model, validation_dataset, input_reader, epoch + 1, updates_epoch
                )
    ...
    def _load_model(self, input_reader):
        ...
        print(f"Loading model: {self._args.model_path}")
    ...
    def _predict(
        self, model: torch.nn.Module, dataset: Dataset, input_reader: BaseInputReader
    ):
        # create data loader
        dataset.switch_mode(Dataset.EVAL_MODE)
        data_loader = DataLoader(
    ...
    def _log_eval(
        ...
        self._log_tensorboard(
            label, "eval/ner_prec_micro", ner_prec_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/ner_recall_micro", ner_rec_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/ner_f1_micro", ner_f1_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/ner_prec_macro", ner_prec_macro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/ner_recall_macro", ner_rec_macro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/ner_f1_macro", ner_f1_macro, global_iteration
        )

        self._log_tensorboard(
            label, "eval/rel_prec_micro", rel_prec_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_recall_micro", rel_rec_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_f1_micro", rel_f1_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_prec_macro", rel_prec_macro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_recall_macro", rel_rec_macro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_f1_macro", rel_f1_macro, global_iteration
        )

        self._log_tensorboard(
            label, "eval/rel_nec_prec_micro", rel_nec_prec_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_nec_recall_micro", rel_nec_rec_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_nec_f1_micro", rel_nec_f1_micro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_nec_prec_macro", rel_nec_prec_macro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_nec_recall_macro", rel_nec_rec_macro, global_iteration
        )
        self._log_tensorboard(
            label, "eval/rel_nec_f1_macro", rel_nec_f1_macro, global_iteration
        )
