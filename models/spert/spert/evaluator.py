...
class Evaluator:
    ...
    def store_examples(self):
        ...
        for i, doc in enumerate(self._dataset.documents):
            ...
            entity_example = self._convert_example(
                doc,
                self._gt_entities[i],
                self._pred_entities[i],
                include_entity_types=True,
                to_html=self._entity_to_html,
            )
            entity_examples.append(entity_example)

            # relations
            # without entity types
            rel_example = self._convert_example(
                doc,
                self._gt_relations[i],
                self._pred_relations[i],
                include_entity_types=False,
                to_html=self._rel_to_html,
            )
            rel_examples.append(rel_example)

            # with entity types
            rel_example_nec = self._convert_example(
                doc,
                self._gt_relations[i],
                self._pred_relations[i],
                include_entity_types=True,
        ...
        self._store_examples(
            entity_examples[: self._example_count],
            file_path=self._examples_path % "entities",
            template="entity_examples.html",
        )

        self._store_examples(
            sorted(entity_examples[: self._example_count], key=lambda k: k["length"]),
        ...
        # without entity types
        self._store_examples(
            rel_examples[: self._example_count],
            file_path=self._examples_path % "rel",
            template="relation_examples.html",
        )

        self._store_examples(
            sorted(rel_examples[: self._example_count], key=lambda k: k["length"]),
        ...
        # with entity types
        self._store_examples(
            rel_examples_nec[: self._example_count],
            file_path=self._examples_path % "rel_nec",
            template="relation_examples.html",
        )

        self._store_examples(
            sorted(rel_examples_nec[: self._example_count], key=lambda k: k["length"]),
    ...
    def _rel_to_html(self, relation: Tuple, encoding: List[int]):
        ...
        if head[0] < tail[0]:
            e1, e2 = head, tail
            e1_tag, e2_tag = (
                head_tag % head[2].verbose_name,
                tail_tag % tail[2].verbose_name,
            )
        else:
            e1, e2 = tail, head
            e1_tag, e2_tag = (
